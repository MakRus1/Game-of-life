...
void Menu::Init_menu() {
	...
	instructions_font = TTF_OpenFont("Vogue Bold.ttf", 60);
	author_font = TTF_OpenFont("Vogue Bold.ttf", 40);
	...
	texture_array[TEXTURE_INDEX_AUTHOR]->Load_From_Renderer_Text(author_font, "Vinogradov Maksim O713B", white);
...
void Menu::Draw() {
	...
	texture_array[TEXTURE_INDEX_INFO]->Draw(
		...
		(SCREEN_HEIGHT - texture_array[TEXTURE_INDEX_INFO]->Get_Height()) / 2);
	texture_array[TEXTURE_INDEX_EXIT]->Draw(
		(SCREEN_WIDTH - texture_array[TEXTURE_INDEX_EXIT]->Get_Width()) / 2,
		(SCREEN_HEIGHT - texture_array[TEXTURE_INDEX_EXIT]->Get_Height()) / 2 + 100);
